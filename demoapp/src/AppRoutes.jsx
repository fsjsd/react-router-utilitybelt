...
import {
  MdHome, MdAttachMoney, MdLocalLibrary, MdHelp, MdSettings,
  MdReceipt, MdAssessment, MdPieChart, MdDescription, MdPermContactCalendar,
...
const appRoutes = {
  '/': makeRouteDefinition({
    title: 'Summary', navlabel: 'Home', icon: MdHome, component: Stubs.Home, exact: true,
  }),
  '/catalog': makeRouteDefinition({
    title: 'Catalog', icon: MdLocalLibrary, component: Stubs.Catalog,
  }),
  '/orders': makeRouteDefinition({
    title: 'Orders', icon: MdReceipt, component: Stubs.Orders,
  }),
  '/billing': makeRouteDefinition({
    title: 'Billing', icon: MdAttachMoney, component: Stubs.Billing,
  }),
  '/portfolio': makeRouteDefinition({
    title: 'Portfolio', icon: MdAssessment, component: Stubs.Portfolio,
  }),
  '/charts': makeRouteDefinition({
    title: 'Charts', icon: MdPieChart, component: Stubs.Charts, group: toolsGroup,
  }),
  '/reports': makeRouteDefinition({
    title: 'Reports', icon: MdDescription, component: Stubs.Reports, group: toolsGroup,
  }),
  '/report/:reportId': makeRouteDefinition({
    title: '', icon: MdDescription, component: Stubs.Report, group: toolsGroup, showInNav: false, parent: '/reports',
  }),
  '/form': makeRouteDefinition({
    title: 'Form Demo', icon: MdPermContactCalendar, component: Stubs.FormExample, group: toolsGroup,
  }),
  '/support': makeRouteDefinition({
    title: 'Support', icon: MdHelp, component: Stubs.Support, group: helpGroup,
  }),
  '/breadcrumbs': makeRouteDefinition({
    title: 'Breadcrumbs demo', icon: MdSettings, component: Stubs.Breadcrumbs, exact: true, group: demosGroup, showSubNav: true,
  }),
  '/breadcrumbs/1': makeRouteDefinition({
    title: 'Level 1', icon: MdSettings, component: Stubs.Breadcrumbs1, parent: '/breadcrumbs', exact: true, group: demosGroup,
  }),
  '/breadcrumbs/1/2': makeRouteDefinition({
    title: 'Level 2', icon: MdSettings, component: Stubs.Breadcrumbs2, parent: '/breadcrumbs/1', exact: true, group: demosGroup,
  }),
  '/settings': makeRouteDefinition({
    title: 'Settings', icon: MdSettings, component: Stubs.Settings, showInNav: false,
  }),
};
